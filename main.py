...
from bullet import Bullet, Check, YesNo, Input, colors, utils
...
def get_total_order_amount(order: List[PurchaseItem]):
    ...
    raise NotImplementedError("REMOVE this line and RETURN the total amount for the order")
...
def get_service_charge(order: List[PurchaseItem]):
    ...
    raise NotImplementedError("REMOVE this line and RETURN service charge amount for the order")    
...
def print_order(order):
    ...
    utils.cprint("Final Order", color=colors.foreground["green"], on=colors.background["yellow"])
    for i, item in enumerate(order):
        utils.cprint(f"{i+1}. {item.name}", color=colors.foreground["yellow"], on=colors.background["green"])
 
    utils.cprint(f"Order Amount: {str(total_amount)}", color=colors.foreground["green"], on=colors.background["yellow"])
    utils.cprint(f"Service Charge: {str(service_charge)}", color=colors.foreground["green"], on=colors.background["yellow"])
    utils.cprint(f"Final Amount: {str(total_amount + service_charge) if type(total_amount) is int and type(service_charge) is int else 'ERROR'}", color=colors.foreground["green"], on=colors.background["yellow"])
order = []
while True:
    options = list(map(lambda x: str(x), MCDONALDS_FOOD_OPTIONS))
    bullet = Bullet(prompt = "Add an item", choices=options)
    ...
    utils.cprint(f"{result} is added to your order", on=colors.background["green"], end = '\n')

# utils.clearConsoleUp(1)
while True:
    options = list(map(lambda x: str(x), MCDONALDS_BEVERAGES_OPTIONS))
    bullet = Bullet(prompt = "Add a beverage", choices=options)
    result = bullet.launch()
    utils.clearConsoleUp(7)
